...
public class SubroutineHierarchy extends JPanel {
	...
	private void rotatingCircle(Graphics2D g2, double x, double y, double r) {
		...
		Color saveColor = g2.getColor();
		Point2D[] list = new Point2D[17];
		double animationOffset = frameNumber/20.0;
		for(int i = 0; i<18; i++) {
			list[i] = new Point2D.Double(
					(r/2)*Math.cos(animationOffset + (i*2*Math.PI/17)) + x,
					(r/2)*Math.sin(animationOffset + (i*2*Math.PI/17)) + y);
		}
		Point2D center = new  Point2D.Double(x, y);
		for(int i = 0; i<18; i++) g2.draw(new Line2D.Double(center, list[i]));
		for(int i = 1; i<18; i++) g2.draw(new Line2D.Double(list[i-1], list[i]));
		g2.setColor(saveColor);
		g2.setTransform(saveTransform);
	}
